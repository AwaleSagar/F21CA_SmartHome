...
#ngrok_url="http://315a138c.ngrok.io/"#diu['tunnels'][0]['public_url']
ngrok_url="http://15e9aef9.ngrok.io/" # %22%7D"
#-----------------------CALL BOT---------------------------------------------------------
...
class MainWindow():
    ...
    def askAlana(self,text):
        ...
        #text=self.variable1.get()
        data = {'user_id': 'test-5827465823641856215', 'question': text, 'session_id': 'CLI-1100002', 'projectId': 'CA2020', 'overrides': {'BOT_LIST': bot_list_ , 'PRIORITY_BOTS': [prio_bot_list_]}}
        ...
        #sentence_list = ['For the ozone concentration, air quality is considered good. Concerning the carbon monoxide concentration, the measure is unavailable. Finally, for the nitrogen dioxide concentration, air quality is satisfactory, with little to no risk posed by air pollution.']
        ...
        for i in range (0,len(sentence_list)):            
            ...
            try:
                ...
                mixer.init()
                #mixer.music.load("speech12.mp3")
                #playsound('speech1.mp3', True)
                #os.remove("speech1.mp3")
                #mixer.music.play()
                
                #son1= open("speech"+str(numbermovie)+".mp3","rb")
                son1= open("speech.mp3","rb")
                mixer.music.load(son1)
                mixer.music.play()
